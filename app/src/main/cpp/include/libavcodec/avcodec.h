...
#define AV_CODEC_ID_H265 AV_CODEC_ID_HEVC
  ...
  AV_CODEC_ID_PROBE =
  0x19000, ///< codec_id is not known (like AV_CODEC_ID_NONE) but lavf should attempt to identify it
  ...
  0x21000,   ///< Dummy codec for streams containing only metadata information.
...
typedef struct AVPacket {
  ...
  int64_t
      pos;                            ///< byte position in stream, -1 if unknown
...
#define FF_THREAD_SLICE   2 ///< Decode more than one part of a single frame at once
  ...
  int (*execute2)(struct AVCodecContext *c,
                  int (*func)(struct AVCodecContext *c2,
                              void *arg,
                              int jobnr,
                              int threadnr),
                  void *arg2,
...
av_codec_get_codec_descriptor(const AVCodecContext *avctx);
attribute_deprecated
...
typedef struct AVCodec {
  ...
  int capabilities;
  const AVRational *
      supported_framerates; ///< array of supported framerates, or NULL if any, array is terminated by {0,0}
  ...
  uint8_t
      max_lowres;                     ///< maximum value for lowres supported by the decoder
...
typedef struct AVHWAccel {
   ...
   */
  int
  (*start_frame)(AVCodecContext *avctx, const uint8_t *buf, uint32_t buf_size);
   ...
   * @return zero if successful, a negative value otherwise
   */
  int (*decode_params)(AVCodecContext *avctx,
                       int type,
                       const uint8_t *buf,
                       uint32_t buf_size);
   ...
   */
  int
  (*decode_slice)(AVCodecContext *avctx, const uint8_t *buf, uint32_t buf_size);
 ...
 */
int
avcodec_parameters_copy(AVCodecParameters *dst, const AVCodecParameters *src);
 ...
 */
uint8_t *
av_packet_get_side_data(const AVPacket *pkt, enum AVPacketSideDataType type,
                        int *size);
 ...
 */
int
av_packet_unpack_dictionary(const uint8_t *data, int size, AVDictionary **dict);
 ...
 */
enum AVPixelFormat
avcodec_find_best_pix_fmt_of_list(const enum AVPixelFormat *pix_fmt_list,
                                  enum AVPixelFormat src_pix_fmt,
