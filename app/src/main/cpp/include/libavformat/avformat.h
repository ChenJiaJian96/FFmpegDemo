...
typedef struct AVProbeData {
  const char *filename;
  unsigned char *
      buf; /**< Buffer must have AVPROBE_PADDING_SIZE of extra allocated bytes filled with zero. */
...
#define AVINDEX_DISCARD_FRAME  0x0002    /**
  ...
  int
      min_distance;         /**< Minimum distance between this and the previous keyframe, used to avoid unneeded searching. */
} AVIndexEntry;
...
typedef struct AVChapter {
  ...
  AVRational
      time_base;   ///< time base in which the start/end timestamps are specified
...
typedef int(*AVOpenCallback)
    (struct AVFormatContext *s, AVIOContext **pb, const char *url, int flags,
...
av_format_get_control_message_cb(const AVFormatContext *s);
attribute_deprecated
void av_format_set_control_message_cb(AVFormatContext *s,
                                      av_format_control_message callback);
#if FF_API_OLD_OPEN_CALLBACKS
attribute_deprecated AVOpenCallback
av_format_get_open_cb(const AVFormatContext *s);
attribute_deprecated void
av_format_set_open_cb(AVFormatContext *s, AVOpenCallback callback);
#endif
#endif
...
av_probe_input_format(ff_const59 AVProbeData *pd, int is_opened);
 ...
 */
ff_const59 AVInputFormat *av_probe_input_format2(ff_const59
                                                 AVProbeData *pd,
                                                 int is_opened,
                                                 int *score_max);
 ...
 */
ff_const59 AVInputFormat *av_probe_input_format3(ff_const59
                                                 AVProbeData *pd,
                                                 int is_opened,
                                                 int *score_ret);
 ...
 */
AVProgram *
av_find_program_from_stream(AVFormatContext *ic, AVProgram *last, int s);

void
av_program_add_stream_index(AVFormatContext *ac, int progid, unsigned int idx);
 ...
 */
enum AVCodecID
av_guess_codec(ff_const59 AVOutputFormat *fmt, const char *short_name,
 ...
 */
void
av_pkt_dump_log2(void *avcl, int level, const AVPacket *pkt, int dump_payload,
                 const AVStream *st);
 ...
 */
enum AVCodecID
av_codec_get_id(const struct AVCodecTag *const *tags, unsigned int tag);
 ...
 */
unsigned int
av_codec_get_tag(const struct AVCodecTag *const *tags, enum AVCodecID id);
 ...
 */
AVRational
av_guess_frame_rate(AVFormatContext *ctx, AVStream *stream, AVFrame *frame);
