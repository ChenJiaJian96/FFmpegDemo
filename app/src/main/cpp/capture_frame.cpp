...
int invoke_bitmap_callback(JNIEnv *env,
  ...
  jmethodID method_id = env->GetMethodID(javaClass,
                                         "onBitmapCaptured",
                                         "(IJLandroid/graphics/Bitmap;Z)V");
  if (method_id == nullptr) {
...
Java_com_igniter_ffmpegtest_VideoManager_capture(JNIEnv
*env,
jobject thiz,
    jstring
video_path,
...
invoke_step_passed(env, on_bitmap_callback_listener,
0, 0);
for (
int index = 0;
index<total_num;
...
// endregion
// decode finished
invoke_step_passed(env, on_bitmap_callback_listener, index
+ 1, 1);
