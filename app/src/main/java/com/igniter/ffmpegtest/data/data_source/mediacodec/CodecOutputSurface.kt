 ...
 */
class CodecOutputSurface(val width: Int, val height: Int) : SurfaceTexture.OnFrameAvailableListener {

    companion object {
        const val NEW_IMAGE_WAIT_TIMEOUT_MS = 2500
    }
    private lateinit var mTextureRender: STextureRender
    ...
    lateinit var surface: Surface
        private set
    private lateinit var mPixelBuf: ByteBuffer // used by saveFrame()
        ...
        synchronized(mFrameSyncObject) {
            while (!mFrameAvailable) {
                try {
                    ...
                    mFrameSyncObject.wait(NEW_IMAGE_WAIT_TIMEOUT_MS.toLong())
                    if (!mFrameAvailable) {
